...
excellence_scores <-
  ...
  judging %>%
  filter(!is.na(notebook_score)) %>%
  filter(notebook_score != 0) %>%
  # must have an interview
  filter(!is.na(interview_score)) %>%
  filter(interview_score != 0) %>%
  # must be in the top 5 or 20% of skills scores
  inner_join(
    skills_data %>%
      arrange(skills_rank) %>%
      slice_head(n = excellence_skills_count),
    by = "team"
  ) %>%
  inner_join(
    ranked_qualifer_results %>%
      slice_head(n = excellence_qualifiers_count),
    by = "team"
  ) %>%
  rowwise() %>%
  # mutate(excellence_score = sum(skills_rank, qualifier_rank, judged_rank)) %>%
  arrange(
    # excellence_score,
    judged_rank,
    notebook_rank,
...
The Excellence ranking combining Judging, Skills and Qualifiers is
